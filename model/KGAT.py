...
class Aggregator(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dropout, aggregator_type):
        ...
        self.message_dropout = nn.Dropout(dropout)    # dropout : 0.1
        self.activation = nn.LeakyReLU()

        if self.aggregator_type == 'gcn':
            self.linear = nn.Linear(self.in_dim, self.out_dim)       # W in Equation (6)
            nn.init.xavier_uniform_(self.linear.weight)              # 가중치 초기화
    ...
    def forward(self, ego_embeddings, A_in):
        ...
        """
        # Equation (3)
        side_embeddings = torch.matmul(A_in, ego_embeddings)    # 이웃 엔티티만 활성화
...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
                 ...
                 entity_embed=None, embeddings=None):
        ...
        self.entity_user_embed = nn.Embedding(self.n_entities + self.n_users, self.embed_dim)             # 엔티티+유저 임베딩
        self.relation_embed = nn.Embedding(self.n_relations, self.relation_dim)                           # 관계 임베딩
        self.trans_M = nn.Parameter(torch.Tensor(self.n_relations, self.embed_dim, self.relation_dim))    # W 정의

        if (self.use_embedding == 1) and (entity_embed is not None):
            user_embed = nn.Parameter(torch.Tensor(self.n_users, self.embed_dim))
            nn.init.xavier_uniform_(user_embed)
            entity_user_embed = torch.cat([entity_embed, user_embed], dim=0)
            self.entity_user_embed.weight = nn.Parameter(entity_user_embed)
        else:
            nn.init.xavier_uniform_(self.entity_user_embed.weight)    # 가중치 초기화

        nn.init.xavier_uniform_(self.relation_embed.weight)
        nn.init.xavier_uniform_(self.trans_M)

        self.aggregator_layers = nn.ModuleList()    # 모듈을 리스트 형태로 저장
        for k in range(self.n_layers):
            self.aggregator_layers.append(Aggregator(self.conv_dim_list[k], self.conv_dim_list[k + 1], self.mess_dropout[k], self.aggregation_type))    # conv_dim_list : [64,32,16]

        self.A_in = nn.Parameter(torch.sparse.FloatTensor(self.n_users + self.n_entities, self.n_users + self.n_entities))    # Adjacency matrix (sparse 최적화)
        if A_in is not None:
            self.A_in.data = A_in
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):    # 정보 전파
    ...
    def calc_score(self, user_ids, item_ids):
        ...
        # Add BERT embedding to item embedding
        if self.embeddings is not None:
            additional_embedding = self.embeddings[item_ids].float()
            additional_embedding = F.normalize(additional_embedding, p=2, dim=1)
            item_embed = torch.add(item_embed, additional_embedding)
