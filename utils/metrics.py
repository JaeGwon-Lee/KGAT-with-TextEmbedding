...
def recall_at_k_batch(hits, k):
    ...
    # 데이터가 너무 크면 sum 계산이 안돼서 nan값을 출력함 => 100개씩 계산
#     total = 0
#     tmp = 0
#     stride = 100
#     hits_len = hits.shape[1]
#     while hits_len > tmp:
#         tmp += stride
#         total += hits[:, tmp-stride:tmp].sum(axis=1)
    
#     res = (hits[:, :k].sum(axis=1)) / total
#     print(hits[:, :k].sum(axis=1))
#     print(total)
#     print(res)
    return res
...
def calc_metrics_at_k(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks):
    ...
    for idx, u in enumerate(user_ids):
        ...
        cf_scores[idx][train_pos_item_list] = -np.inf    # cf_scores 행렬에서 train pos item의 점수를 -inf값으로 변경 (train pos item들은 예측에서 제외)
        test_pos_item_binary[idx][test_pos_item_list] = 1    # test_pos_item_binary 행렬에서 test pos item의 점수를 1로 설정

    try:
        _, rank_indices = torch.sort(cf_scores.cuda(), descending=True)    # cf_scores 행렬을 점수가 높은 순으로 정렬한 인덱스
    except:
    ...
    for i in range(len(user_ids)):
        binary_hit.append(test_pos_item_binary[i][rank_indices[i]])    # score 순으로 정렬된 정답 여부(0 or 1)
        predict_dict[i] = rank_indices[i][:10].tolist()
    ...
    return metrics_dict, predict_dict
