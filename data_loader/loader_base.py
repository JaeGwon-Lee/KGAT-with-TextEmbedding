...
class DataLoaderBase(object):
    ...
    def __init__(self, args, logging):
        ...
        self.data_dir = os.path.join(args.data_dir, args.data_name, f'user{args.n_user}_{args.nlp}_{args.use_embedding}_{args.data_size}')
        ...
        self.cf_train_data, self.train_user_dict = self.load_cf(self.train_file)    # output 형태 => data : (user array, item array) / user_dict : {user: [item list]}
        self.cf_test_data, self.test_user_dict = self.load_cf(self.test_file)
        self.statistic_cf()


    # user-item interaction 데이터 로드
    def load_cf(self, filename):
        ...
        for key in list(user_dict.keys()):
            user_dict[int(key)] = user_dict.pop(key)
            user.extend([int(key)]*len(user_dict[int(key)]))    # 아이템 당 유저 하나씩 추가
            item.extend(user_dict[int(key)])
    ...
    # user-item interaction 데이터 개수 통계
    ...
    def generate_cf_batch(self, user_dict, batch_size):
        ...
        if batch_size <= len(exist_users):
            batch_user = random.sample(exist_users, batch_size)    # 전체 user에서 batch size만큼 샘플링
        else:
            batch_user = [random.choice(exist_users) for _ in range(batch_size)]

        batch_pos_item, batch_neg_item = [], []
        for u in batch_user:
            batch_pos_item += self.sample_pos_items_for_u(user_dict, u, 1)   # user u가 선호하는 아이템 1개 샘플링
    ...
    def generate_kg_batch(self, kg_dict, batch_size, highest_neg_idx):
        ...
        if batch_size <= len(exist_heads):
            batch_head = random.sample(exist_heads, batch_size)    # 전체 head에서 batch size만큼 샘플링
        else:
            batch_head = [random.choice(exist_heads) for _ in range(batch_size)]

        batch_relation, batch_pos_tail, batch_neg_tail = [], [], []
        for h in batch_head:
            relation, pos_tail = self.sample_pos_triples_for_h(kg_dict, h, 1)   # head h와 연결되는 relation과 tail 1개 샘플링
